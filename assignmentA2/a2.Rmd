...
tf <- rep(0, (n + m + K))
for (i in 1:(m+K)){
  tf[i+n] <- delta^(i-1) + tf[(i+n-1)] - tf[(i+n-k-1)]
}
tf <- omega_0*tf
...
Percebemos que o MAPE de previsão é ligeiramente maior nesse modelo, e, portanto, o primeiro modelo estimado ainda parece ter um bom comportamento. 