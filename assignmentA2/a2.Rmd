...
A tabela mostra vários critérios de informação para os modelos $\text{ARMA}(p, q)[12]$ e $\text{ARMA}(p, q)[12]$ com média movel sazonal (S).
...
Observamos que as previsões de nosso modelo ficam em um patamar mais alto do que os dados reais e por isso tomamos como hipótese de que a crise foi significante. Consideramos os procedimentos sugeridos por Box e Tiao (1976) com a checagem do erro de previsão. 

Considere a estatística $$Q = \sum_{j=1}^m a_j^2/\hat{\sigma}_a^2$$ onde $a_j = Z_j - \hat{Z}_{j-1}(1), j = 1, ..., m$. Sabemos que $Q$ seque uma distribuição $\chi^2(m)$ quando $m$ é grande. 
...
Vamos agora realizar a previsão até 24 passos à frente.

```{r, echo = F}

# https://stats.stackexchange.com/q/169564

delta = as.numeric(mod.arimax$coef['Crise-AR1'])
omega_0 = as.numeric(mod.arimax$coef['Crise-MA1'])
tf = rep(0, length(pre_intervention))
for (i in 1:length(pos_intervention)) {
  tf = c(tf, delta^(i-1)*omega_0)
}
tf = tf*as.numeric(mod.arimax$coef['Crise-MA0'])
...
autoplot(forecast(mod.arima, h=24, xreg = rep(0, 24)),
         level = c(90, 95),
...
Vamos verificar o MAPE da nossa previsão:

```{r, echo = F}
mape(tbs_test, forecast(mod.arima, h=24, xreg = rep(0, 24))$mean)
```
...
<!-- ```{r, echo = F} -->
<!-- autoplot(window(tbs, start = c(2005, 1), end = c(2012,12)), main = "Total de vendas nos negócios",  -->
<!--               xlab = "Tempo",  -->
<!--               ylab = "Milhões de dólares") +  -->
<!--   geom_vline(xintercept = as.Date("2008-07-01", "%Y-%m-%d"),  -->
<!--              color = 'red',  -->
<!--              linetype = "dashed", -->
<!--              size = 0.5) + -->
<!--   geom_vline(xintercept = as.Date("2009-01-01", "%Y-%m-%d"),  -->
<!--              color = 'red',  -->
<!--              linetype = "dashed", -->
<!--              size = 0.5) -->
<!-- ``` -->
...
<!--                                           (seq(tbs_train) >= length(pre_intervention) + 1 &  -->
<!--                                             seq(tbs_train) <= length(pre_intervention) + 1 + 6)), -->
<!--                      transfer=list(c(1, 1)), -->
<!--                      method = "ML") # problemas com CSS-ML e CSS, porquê? -->
<!-- summary(mod2.arimax) -->
<!-- ``` -->
...
<!-- Visualmente, os resíduos se comportam de forma melhor que anteriormente: a distorção na intervenção foi amenizada e a ACF de comporta melhor. -->
...
<!-- Ainda assim, percebemos que esse modelo reflete melhor a intervenção na crise do subprime. -->
