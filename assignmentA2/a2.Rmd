...
Ainda assim, percebemos que esse modelo reflete melhor a intervenção na crise do subprime. Podemos conferir como o modelo se comporta com os daos reais. 
...
omega_0 <- as.numeric(mod2.arimax$coef['Crise-MA0'])
...
```{r}
autoplot(forecast(mod2.arima, h=24, xreg = tf[(n+m+1):(n+m+K)]),
         level = c(90, 95),
         main = "Total de vendas nos negócios",
         xlab = "Tempo",
         ylab = "Milhões de dólares")
...
```{r, echo = F}
mape(tbs_test, forecast(mod2.arima, h=24, xreg = tf[(n+m+1):(n+m+K)])$mean)
```

Percebemos que o MAPE de previsão é ligeiramente menor nesse modelo, e, portanto, o segundo modelo estimado ainda parece ter um bom comportamento. 