...
gridExtra::grid.arrange(p1,p2,p3, nrow = 1, top = 'Métricas de Erro')
```
...
sendo $X_t$ ``temporário'' e $v(B) = \dfrac{\omega_0}{1 - \delta B}$. Portanto, 

$$h_t = v(B) X_t = \begin{cases}
0, &t < \text{jul 2018} \\
\delta^k \omega_0, &t = \text{jul 2008} + k, \ \ k = 0, 1, 2, \cdots
\end{cases}$$

Estimaremos a série com intervenção, ou seja, também estimaremos os parâmetros $\delta$ e $\omega_0$.
...
Vamos agora realizar a previsão até 24 passos à frente. Para isso precisamos usar as estimativas de $\delta$ e $\omega_0$ dados pelo arimax e calculamos $h_t$. Esses valores são usados como regressores da função `Arima`, a qual podemos fazemos fazer predições. 

```{r, echo = F}
delta <- as.numeric(mod.arimax$coef['Crise-AR1'])
omega_0 <- as.numeric(mod.arimax$coef['Crise-MA0'])
tf <- rep(0, (n + m + K))
for (i in 1:(m+K)){
  tf[i+n] <- delta^(i-1)*omega_0
}
...
```{r, echo=F}
autoplot(forecast(mod.arima, h=24, xreg = tf[(n+m+1):(n+m+K)]),
         level = c(90, 95),
...
```{r, echo = F}
mape(tbs_test, forecast(mod.arima, h=24, xreg = tf[(n+m+1):(n+m+K)])$mean)
```

Nos parece um ótimo resultado.

Porém, acreditamos que podemos melhorar.
É óbvio que o efeito da intervenção não é permanente, por isso optamos pelo que chamamos de efeito temporário.
Porém, observe que o efeito da intervenção também não é imediato:
...
Isso nos indica que poderíamos considerar um efeito temporário, mas não imediato, de alguns meses.

Vamos modelar a mesma intervenção, porém permitindo que ela dure mais tempo, por 6 meses (desde jul 2008 até dez 2008).
...
O teste de Ljung-Box rejeitou a hipótese de descorrelação, o que não é um bom sinal.

Para tentar corrigir esses problemas, podemos testar um modelo um pouco mais complexo com um termo de média móvel na intervenção. 

```{r, echo=F}
mod3.arimax <- arimax(tbs_train.bc,
                     order = c(2, 1, 0),
                     seasonal = c(0, 1, 1),
                     xtransf=data.frame(Crise=1*
                                       ((seq(tbs_train) >= n + 1) &
                     ...
                     method = "ML")
summary(mod3.arimax)
checkresiduals(mod3.arimax)
```

Percebemos que esse modelo reflete a normalidade dos resíduos de forma melhor, pelo histograma. Ainda não foi possível retirar toda a autocorrelação do modelo, rejeitando a não existência de correlação nos dados pelo teste de Ljung-Box. Podemos, entretanto, comparar os erros dos três modelos. 

```{r, echo = F}
model.errors <- data.frame()
model.errors <- rbind(model.errors, 
                      error.metrics(tbs_train.bc, fitted(mod.arimax), 1))
colnames(model.errors) <- c('Modelo', 'MAPE', 'MAE', 'RMSE')
model.errors <- rbind(model.errors, 
                      error.metrics(tbs_train.bc, fitted(mod2.arimax), 2))
model.errors <- rbind(model.errors, 
                      error.metrics(tbs_train.bc[-1], fitted(mod3.arimax)[-1], 3))
...
Nesse sentido, o terceiro modelo apresenta, nas três métricas, um resíduo menor e, por isso, ele melhor reflete a intervenção na crise do subprime. Podemos conferir como o modelo se comporta com os dados reais. 

```{r, echo = F, warning = F, message=F}
x <- time(tbs_train.bc)
tbs_train.bc_df <- data.frame(x = x, y = tbs_train.bc)
fitted_model <- fitted(mod3.arimax)
...
# Previsão com o segundo modelo de intervenção

O gráfico abaixo nos permite visualizar a predição do modelo até até 24 passos à frente.

```{r, echo = F}
k <- 6 # size of the effect 

delta <- as.numeric(mod3.arimax$coef['Crise-AR1'])
omega_0 <- as.numeric(mod3.arimax$coef['Crise-MA0'])
omega_1 <- as.numeric(mod3.arimax$coef['Crise-MA1'])

tf.tmp <- tf <- rep(0, (n + m + K))
for (i in 1:(m+K)){
  tf.tmp[(i+n)] <- delta^(i-1)
}
for (i in 1:(m+K)){
  tf[(i+n)] <- sum(tf.tmp[(i+n-k):(i+n)])
}
tf <- (omega_0*delta + omega_1)*tf
for (i in 1:k){
  tf[(n+i)] <- tf[(n+i)] + omega_0 
}

mod3.arima <- Arima(tbs_train, 
                    order = c(2,1,0), 
                    seasonal = c(0,1,1),
                    lambda = 0,
                    xreg = tf[1:(n+m)])
...
```{r, echo=F}
autoplot(forecast(mod3.arima, h=24, xreg = tf[(n+m+1):(n+m+K)]),
         level = c(90, 95),
         main = "Total de vendas nos negócios",
         xlab = "Tempo",
         ylab = "Milhões de dólares")
...
E o MAPE para o erro de previsão é o seguinte:

```{r, echo = F}
mape(tbs_test, forecast(mod3.arima, h=24, xreg = tf[(n+m+1):(n+m+K)])$mean)
```

Percebemos que o MAPE de previsão é bem maior do que o primeiro modelo e, portanto, não parece ter sido uma boa ideia para essa intervenção. 