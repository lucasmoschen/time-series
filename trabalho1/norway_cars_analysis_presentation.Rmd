...
r <- rollapply(make_df.ts, FUN = es, 
               width = 24, align = 'right')

mape_exp <- mean(abs((make_df.ts - r)/make_df.ts))
```
...
hw <- rollapply(make_df.ts, FUN = es, 
               width = 24, align = 'right')

mape_hw <- mean(abs((make_df.ts - r)/make_df.ts))
```
