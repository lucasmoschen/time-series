...
Now we analyse the residuals. Let's see the ACF and PACF:
...
As before, wee see spikes in $\text{lag} = 365, 730$. We expect a residual to not have this.
...
Based on these graphs, we see both graphs has a exponentially decay, the first after the $p - q = 2$. In order to identify the model, we will compare the adjusted ARMA models with different $p$ and $q$. 
...
The third model has no `p-value` of the estimated coefficients are no statistically sigificant, so it's not a good model. The first and secind models have all coefficients significant and the AIC criterion shows the second to be better. So let's use the second one with the rollyapply. 
