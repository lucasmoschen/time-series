...
2. Escrever melhor o texto em algumas transições. 

3. Alguma combinação de modelo?

4. Residual analysis with adjusted error, not predictive one.
...
mae <- c(stl_mul_mae, regression_mae, hw_mult_mae, arma_mae)
mae_all <- data.frame(Model = models, MAE = mae)
ggplot(mae_all) + 
...
baseline_model_week <- function(t) {
  return(as.numeric(o3_train_week[(size-1)]))
}
...
```{r, echo = F}
par(mfrow = c(1,2))
acf(o3_train_week[size:n]-prediction, lag.max = size, main = 'ACF Remainder')
pacf(o3_train_week[size:n]-prediction, lag.max = size, main = 'PACF Remainder')
```
...
We see the same problems as before: this is not white noise.
...
```{r arma-2-week, echo=F, warning = F}
arma_model_week <- function(t) {
...
output(arma_mae, "ARIMA(2,0,1) prediction", prediction, o3_test_week)
```
