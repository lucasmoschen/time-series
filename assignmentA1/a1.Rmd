...
2. Compare the daily models: we use rollyaaply using 2 years + 6 days to predict the day, that is, we use $\{X_t\}_{t-2\cdot365}^t$ to predict $X_{t+7}$. We calcute, for each $t > 2\cdot365 + 6$, $|\hat{X}_{t} - X_t|$ and after calculate $\sum_t |\hat{X}_{t} - X_t|$. We used Decomposition, Regression, Holt-Winters and ARMA. 
3. Compare the weekly models: we use rollyapply using 2 years + 1 week to predict the week, that is, we use $\{X_t\}_{t-2\cdot52}^t$ to predict $X_{t+1}$. We calcute, for each $t > 2\cdot52 + 1$, $|\hat{X}_{t} - X_t|$ and after calculate $\sum_t |\hat{X}_{t} - X_t|$. We used Decomposition, Regression, Holt-Winters and ARMA. 
...
output = function(mae, title, prediction, data) {
  print(mae)
  plot(time(o3)[1:n], data,
       main = title, xlab = "t")
  lines(time(o3)[size:n], prediction, col="red", lwd = 1)
  legend(
    x = time(o3)[1],
    y = 0.07,
...
output(baseline_mae, "Baseline model prediction", prediction, o3_train)
```
...
output(stl_add_mae, "Additive decompose prediction", prediction[,7], o3_train)
```
...
output(stl_mul_mae, "Multiplicative decompose prediction", exp(prediction[,7]) - 1, o3_train)
```
...
output(regression_mae, "Regression prediction", prediction, o3_train)
```
...
output(hw_add_mae, "Additive Holt-Winters prediction", prediction[,7], o3_train)
```
...
We see the same problems as before: high correlated $\text{lag}=365, 730$, evidence of this not beeing a WN.
...
output(hw_mult_mae, "Multiplicative Holt-Winters prediction", prediction[,7] - 1, o3_train)
```
...
```{r, echo=F}
summary(arma(o3_train, order = c(1,2)))
...
output(arma_mae, "ARIMA(3,0,1) prediction", prediction[,7], o3_train)
```

And the MAE of the ARIMA(3, 0, 2):

```{r arma-2, echo=F, warning = F}
arma_model_day <- function(t) {
  model <- arima(o3_train[t[1:(size-7)]], order = c(1,0,2))
  prediction = forecast(model, h=7)
  return(as.numeric(prediction$mean))
}
...
output(arma_mae, "ARIMA(3,0,2) prediction", prediction[,7], o3_train)
```
...
output(stl_arma_mae, "STL + ARIMA prediction", prediction[,7], o3_train)
```
...
output(stl_mul_mae, "Multiplicative decompose prediction", exp(prediction[,7]) - 1, o3_test)
```
...
output(regression_mae, "Regression prediction", prediction, o3_test)
```
...
output(hw_mult_mae, "Multiplicative Holt-Winters prediction", prediction[,7] - 1, o3_test)
```
...
output(arma_mae, "ARIMA(3,0,1) prediction", prediction[,7], o3_test)
```

Finally, we have this bar graphic to compare the values: 

```{r, echo=FALSE}
models = c('STL', 'Regression', 'HW', 'ARIMA')
mae_result <- c(stl_mul_mae, regression_mae, hw_mult_mae, arma_mae)
...
ggplot(mae_all) + 
  ...
  labs(x = 'Modelos', y = 'MAE', 
      title = 'Comparando MAE para os modelos') + 
...
baseline_model_week <- function(t) {
  return(as.numeric(o3_train_week[t[(size-1)]]))
}

prediction = rollapply(t, width = size, baseline_model_week)
baseline_mae <- mae(o3_train[size:n], prediction)
  
output_week = function(mae, title, prediction, data) {
  print(mae)
  plot(c(1:n), data,
       main = title, xlab = "t")
  lines(c(size:n), prediction, col="red", lwd = 1)
  legend(
    x = time(o3)[1],
    y = 0.07,
    legend = c('Real', 'Prediction'),
...
output_week(baseline_mae, "Baseline model prediction", prediction, o3_train_week)
```
...
stl_add_model_week <- function(t) {
  model <- stl(ts(o3_train_week[t[1:(size-1)]], frequency = freq), 
               s.window = "periodic", robust = T)
...
stl_add_mae <- mae(o3_train_week[size:n], prediction)

output_week(stl_add_mae, "Additive decompose prediction", prediction, o3_train_week)
```
...
stl_mul_model_week <- function(t) {
  model <- stl(ts(data[t[1:(size-1)]], frequency = freq), 
               s.window = "periodic", robust = T)
  prediction = forecast(model, h=1)
  return(as.numeric(prediction$mean))
}
...
stl_mul_mae <- mae(o3_train_week[size:n], exp(prediction) - 1)


output_week(stl_mul_mae, "Multiplicative decompose prediction", exp(prediction) - 1, o3_train_week)
```
...
regression_model_week <- function(t) {
  ...
  train = data.frame(
    t = t[1:(size-1)],
    o3_train_week = o3_train_week[t[1:(size-1)]],
    Q = Q[t[1:(size-1)]]
  )
...
output_week(regression_mae, "Regression prediction", prediction, o3_train_week)
```
...
hw_add_model_week = function(t) {
  ...
  #   print(t[1]/n)
  # }
  mod = HoltWinters(ts(o3_train_week[t[1:(size-1)]], frequency = 52), beta = T, gamma = T, seasonal = "additive")
  prediction = forecast(mod, 1)
  return(as.numeric(prediction$mean))
}

prediction = rollapply(t, width = size, hw_add_model_week)
hw_add_mae <- mae(o3_train_week[size:n], prediction)

output_week(hw_add_mae, "Additive Holt-Winters prediction", prediction, o3_train_week)
```
...
We see the same problems as before: high correlated $\text{lag}=52, 104$, evidence of this not beeing a WN.
...
hw_mult_model_week = function(t) {
  ...
  #   print(t[1]/n)
  # }
  mod = HoltWinters(ts(data[t[1:(size-1)]], frequency = 52), beta = T, gamma = T, seasonal = "multiplicative")
  prediction = forecast(mod, 1)
  return(as.numeric(prediction$mean))
}

prediction = rollapply(t, width = size, hw_mult_model_week)
hw_mult_mae <- mae(o3_train_week[size:n], prediction - 1)

output_week(hw_mult_mae, "Multiplicative Holt-Winters prediction", prediction - 1, o3_train_week)
```
...
prediction = rollapply(t, width = size, arma_model_week)
arma_mae <- mae(o3_train_week[size:n], prediction)
output_week(arma_mae, "ARIMA(2,0,1) prediction", prediction, o3_train_week)
```
...
arma_model_week <- function(t) {
  model <- arima(o3_train_week[t[1:(size-1)]], order = c(3,0,1))
  prediction = forecast(model, h=1)
  return(as.numeric(prediction$mean))
}
...
stl_arma_model_week <- function(t) {
  model <- stl(ts(o3_train_week[t[1:(size-1)]], frequency = freq), 
               s.window = "periodic", robust = T)
  prediction <- forecast(model, h = 1, method = 'arima')
  return(as.numeric(prediction$mean))
}

prediction = rollapply(t, width = size, stl_arma_model_week)
stl_arma_mae <- mae(o3_train_week[size:n], prediction)
output_week(stl_arma_mae, "STL + ARIMA prediction", prediction, o3_train_week)
```
...
prediction = rollapply(t, width = size, stl_mul_model_week)
stl_mul_mae <- mae(o3_test_week[size:n], exp(prediction) - 1)
output_week(stl_mul_mae, "Multiplicative decompose prediction", exp(prediction) - 1, o3_test_week)
```
...
regression_model_week <- function(t) {
  train = data.frame(
    t = t[1:(size-1)],
    o3_test_week = o3_test_week[t[1:(size-1)]],
    Q = Q[t[1:(size-1)]]
  )
...
output_week(regression_mae, "Regression prediction", prediction, o3_test_week)
```
...
prediction = rollapply(t, width = size, hw_mult_model_week)
hw_mult_mae <- mae(o3_test_week[size:n], prediction - 1)

output_week(hw_mult_mae, "Multiplicative Holt-Winters prediction", prediction - 1, o3_test_week)
```


```{r arma-2-week-test, echo=F, warning = F}
arma_model_week <- function(t) {
  model <- arima(o3_test_week[t[1:(size-1)]], order = c(2,0,1))
  prediction = forecast(model, h=1)
  return(as.numeric(prediction$mean))
}

prediction = rollapply(t, width = size, arma_model_week)
arma_mae <- mae(o3_test_week[size:n], prediction)
output_week(arma_mae, "ARIMA(2,0,1) prediction", prediction, o3_test_week)
```
