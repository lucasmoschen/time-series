...
Podemos observar um decaimento exponencial no gráfico acima que cai no intervalo de significância aapenas após o lag 25. Também podemos observar que os lags 12, 23 e 36 tem picos fora do comum que indicam sazonalidade. 
...
Também podemos fazer uma identificação de um modelo alternativo feito artesanalmente. Primeiro fazemos uma trnsformação Box-Cox com o parâmetro $\lambda$ ótimo. Depois podemos fazer um teste de estacionariedade. Porém o teste ADF acusa que não rejeitamos a hipótese nula a nivel 0.05.
...
Portanto, fazemos uma diferença e refazemos o teste. Nesse caso possuimos a estacionaridade desejada. Como ainda temos a sazonalidade já observada, também fazemos a diferença sazonal e, assim: 
...
Para o modelo alternativo, façamos o mesmo processo. 
...
A ACF dos resíduis está particularmente boa! Além disso os resíduos tem um padrão normal também! Além disso o teste de Ljung-BOx não rejeita a hipótese nula de que não existe correlação. O que é uma boa notícia. Vamos ver o jarque.bera.

```{r, echo=F}
jarque.bera.test(est2$residuals)
```
O teste de Jarque Bera também rejeita a hipótese nula, o que não nos dá muito insight sobre o modelo ideial. 
